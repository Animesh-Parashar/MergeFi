...
contract MergeFiRegistry is Ownable {
    ...
    function addContributor(
        ...
        // Check if this is a new contributor by checking isActive field
        (, , , bool isActive) = pool.contributors(contributor);
        bool isNewContributor = !isActive;

        pool.setContributorAllocation(contributor, allocation, githubUsername);

        if (isNewContributor) {
            repos[repoName].contributorCount++;
            // Mint NFT badge for new contributor using the correct function name
            nftBadge.mintContributorBadge(contributor, repoName);
        }
    }
        ...
        )
    {
        ...
        (contributorAddresses, allocations, amounts, githubUsernames) = pool
            .getAllContributors();
    }
}
