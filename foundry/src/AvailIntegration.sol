...
contract AvailIntegration {
    ...
    function getPaymentData(
        ...
        uint256 chainId
    ) external returns (uint256 totalFund, PaymentData[] memory payments) {
        (
            address poolAddress,
            uint256 totalAmount,
     ...
     */
    function getContributorPayment(
        string memory repoName,
        address contributor,
        uint256 chainId
    ) external view returns (PaymentData memory payment) {
     ...
     * @return maintainer Repository maintainer address
     * @return totalFund Total funding amount
     * @return contributorCount Number of contributors
     * @return totalAllocated Total allocated percentage
    ...
    function canProcessPayments(
        ...
        external
        view
        returns (bool canProcess, uint256 totalAllocated, uint256 remaining)
    {
        (, , , address poolAddress, , , , ) = registry.getRepoDetails(repoName);
        RewardPool pool = RewardPool(poolAddress);
    ...
    function getCustomPaymentData(
        ...
        require(
            contributors.length == chainIds.length,
