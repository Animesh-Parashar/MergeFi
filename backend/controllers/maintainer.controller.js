  ...
  try {
    ...
    const detailedContributors = await Promise.all(
      contributorsRes.data.map(async (contributor) => {
        try {
          const userRes = await axios.get(`https://api.github.com/users/${contributor.login}`, {
            headers: { Authorization: `Bearer ${github_token}` },
          });
...
export const getRepoContributorsWithWallets = async (req, res) => {
  ...
  try {
    ...
    // Fetch detailed info for each contributor along with their wallet data
    const detailedContributors = await Promise.all(
      contributorsRes.data.map(async (contributor) => {
        try {
          const userRes = await axios.get(`https://api.github.com/users/${contributor.login}`, {
          ...
          // Fetch wallet address and chain from database
    ...
    console.error(`Error fetching contributors with wallets for ${owner}/${repo}:`, err);
    res.status(500).json({
